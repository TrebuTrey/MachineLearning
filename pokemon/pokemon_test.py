...
class DogsVSCats():
    IMG_SIZE = 50
    NONSHINY = "Pokemon_Images/NonShiny"
    SHINY = "Pokemon_Images/Shiny"
    LABELS = {NONSHINY: 0, SHINY: 1} #Creates a list that represents cats as the 1st entry and dogs as the 2nd
    training_data = []
    shinycount = 0
    nonshinycount = 0
    ...
    def make_training_data(self):
        for label in self.LABELS:
            for f in tqdm(os.listdir(label)):
                if "PNG" in f: #only tries to download the .PNG files
                    try:
        ...
        print('Non-Shinies:',dogsvcats.nonshinycount)
        print('Shinies:',dogsvcats.shinycount)
